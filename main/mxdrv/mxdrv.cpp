...
// ;  Command Line D:\FTOOL\dis.x -C2 --overwrite -7 -m 68040 -M -s8192 -e -g
// mxdrv17.x mxdrv17.dis ;          DIS version 2.75
// ;=============================================
...
#define MXDRV_SET 255
#define MXDRV_CLR 0
#define GETBWORD(a) ((((UBYTE*)(a))[0] * 256) + (((UBYTE*)(a))[1]))
#define GETBLONG(a)                                                            \
    ((((UBYTE*)(a))[0] * 16777216) + (((UBYTE*)(a))[1] * 65536) +              \
     (((UBYTE*)(a))[2] * 256) + (((UBYTE*)(a))[3]))
#define PUTBWORD(a, b)                                                         \
    ((((UBYTE*)(a))[0] = ((b) >> 8)), (((UBYTE*)(a))[1] = ((b) >> 0)))
#define PUTBLONG(a, b)                                                         \
    ((((UBYTE*)(a))[0] = ((b) >> 24)),                                         \
     (((UBYTE*)(a))[1] = ((b) >> 16)),                                         \
     (((UBYTE*)(a))[2] = ((b) >> 8)),                                          \
...
static char MXWORK_CREDIT[] =
    "X68k MXDRV music driver version 2.06+17 Rel.X5-S (c)1988-92 "
    "milk.,K.MAEKAWA, Missy.M, Yatsube\nConverted for Win32 [MXDRVg] V2.00a "
...
static void (*MXCALLBACK_OPMINT)(void);
...
static int Initialize(void* mdxbuf_ptr,
                      int mdxbuf_size,
                      void* pdxbuf_ptr,
                      int pdxbuf_size);
...
static void L_PAUSE_(void);

static void
ZeroMemory(void* p, size_t s)
{
    memset(p, 0, s);
}
            ...
            int pdxbuf_size)
{
    ...
    G.MEASURETIMELIMIT =
        (1000 * (60 * 20 - 2)) * (LONGLONG)4000 / 1024; // 20min-2sec
                                                        /*
                                                                ret = X68Sound_Load();
                                                                if ( ret != 0 ) {
                                                                        switch ( ret ) {
                                                                ...
                                                                if ( betw ) {
                                                                        ret = X68Sound_Start( samprate, opmmode+1, 1, betw, pcmbuf,
                                                          late, 1.0 );                                                     } else {                                                     ret = X68Sound_StartPcm( samprate, 1, 1, pcmbuf );
                                                                }
                                                                if ( ret != 0 ) {
                                                                        switch ( ret ) {
    ...
    sound_iocs_init();
    ret = Initialize(mdxbuf_ptr, mdxbuf_size, pdxbuf_ptr, pdxbuf_size);
    if (ret != 0)
    {
    ...
    {
        int clock = 4000000;
        getMXDRVSoundSystemSet().ym2151->setClock(clock);
        sys::initTimer(clock >> 2); // clock / 1024 * 256
    }
...
void
MXDRV_Play(void* mdx, DWORD mdxsize, void* pdx, DWORD pdxsize)
{
    X68REG reg;
...
static void
MXDRV_MeasurePlayTime_OPMINT(void)
{
...
MXDRV_MeasurePlayTime(
    void* mdx, DWORD mdxsize, void* pdx, DWORD pdxsize, int loop, int fadeout)
{
    X68REG reg;
    void (*opmintback)(void);
    ...
    return ((DWORD)(G.PLAYTIME * (LONGLONG)1024 / 4000 + (1 - DBL_EPSILON)) +
            2000);
}
...
void
MXDRV_PlayAt(DWORD playat, int loop, int fadeout
...
OPMINTFUNC(void)
{
    ...
    if (MXCALLBACK_OPMINT)
        MXCALLBACK_OPMINT();
    //	LeaveCriticalSection( &CS_OPMINT );
}

static void
SETOPMINT(void (*func)(void))
{
    OPMINT_FUNC = func;
    X68Sound_OpmInt(&OPMINTFUNC);
}
...
MXDRV(X68REG* reg)
{
    static void (*jumptable[])(void) = {
        L_FREE, L_ERROR, L_SETMDX, L_SETPDX, L_PLAY, L_STOP, L_PAUSE, L_CONT,
        L_08,   L_09,    L_0A,     L_0B,     L_0C,   L_0D,   L_0E,    L_0F,
        L_10,   L_11,    L_12,     L_13,     L_14,   L_15,   L_16,    L_17,
        L_18,   L_19,    L_1A,     L_1B,     L_1C,   L_1D,   L_1E,    L_1F,
    };
...
L0000dc:;
    /*
                                                                                                                    ...
                                                                                                                    move.w  (a0),$0002(a0)
    */
    d1 = D1, d2 = D2, d3 = D3, d4 = D4, d5 = D5, d6 = D6, d7 = D7, a0 = A0,
    a1 = A1, a2 = A2, a3 = A3, a4 = A4, a5 = A5, a6 = (UBYTE*)A6;
    G.L002245 = MXDRV_SET;
    a0_w      = &G.L001e1e[0];
    A1        = &G.L001e17;
    if (*(A1) == 0)
...
L0001d6:;
    ...
    */
    G.L002245 = MXDRV_CLR;
    D1 = d1, D2 = d2, D3 = d3, D4 = d4, D5 = d5, D6 = d6, D7 = d7, A0 = a0,
    A1 = a1, A2 = a2, A3 = a3, A4 = a4, A5 = a5, A6 = (MXWORK_CH*)a6;
...
L000a66:;
    ...
    */
    ...
    if (G.L001e0a != 0)
        goto L000bd0;
    if (KEY.OPT1 && KEY.SHIFT)
        goto L000b16;
    if (KEY.OPT2 && KEY.SHIFT)
        goto L000b0a;
    if (KEY.OPT1 && KEY.CTRL)
        goto L000b04;
    if (KEY.OPT2 && KEY.CTRL)
        goto L000afc;
    if (KEY.OPT1)
        goto L000b6c;
    if (KEY.OPT2)
...
L000e7e(void)
{
    ...
    */
    ...
    static UBYTE PCMVolume[] = {
        0x0f, 0x0f, 0x0f, 0x0e, 0x0e, 0x0e, 0x0d, 0x0d, 0x0d, 0x0c, 0x0c,
        0x0b, 0x0b, 0x0b, 0x0a, 0x0a, 0x0a, 0x09, 0x09, 0x08, 0x08, 0x08,
        0x07, 0x07, 0x07, 0x06, 0x06, 0x05, 0x05, 0x05, 0x04, 0x04, 0x04,
        0x03, 0x03, 0x02, 0x02, 0x02, 0x01, 0x01, 0x01, 0x00, 0x00, 0xff,
    };
...
static void (*L001252[])(void) = {
    ...
    L0012e6, // @@ p
    L00131c, // @@ v
    L001328, L001344, L001364, L00136a, L001372, L001376, L00139a,
    L0013ba, // @@ D
    L0013c6, L0013dc, L001492, L001498, L0014b0, L0014dc, L0014fc,
    L001590, L0015fe, L001656, L00165c, L001694, L001442, L001442,
    L001442, L001442, L001442, L001442, L001442,
};
...
static int
Initialize(void* mdxbuf_ptr, int mdxbuf_size, void* pdxbuf_ptr, int pdxbuf_size)
{
    G.L002220 = mdxbuf_size;
    G.L002224 = pdxbuf_size;

    G.L001ba8 = 0x600;
    static UBYTE buffer[0x600]; // nazo buffer

    //	G.L001e34 = (UBYTE *)malloc( G.L002220 );
    G.L001e34 = (UBYTE*)mdxbuf_ptr;
    if (!G.L001e34)
    {
        return (!0);
    }
