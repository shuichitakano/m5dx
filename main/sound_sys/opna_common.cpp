...
OPNAState::getNote(int ch, const uint8_t* regCache) const
{
    ...
    else if (ch < 9)
    {
        ...
        // fn = f * 2^20/(clock / 144)/2^(blk-1)
        // f = fn * clock/144 * 2^(blk-1) / 2^20
        // key = 12 log_2(f/440)
        float f = fBaseFM_ * (fn << blk) * 0.5f;
        return f == 0 ? -100 : 17.31234f * log(f) - 105.3763f;
    }
...
OPNAState::getVolume(int ch, const uint8_t* regCache) const
{
    ...
    if (ch < 9)
    {
        ...
        int cm  = audio::OPNSlot::getCarrierMask(al);
...
// const char*
// OPNAState::getStatusString(int ch,
//                            char* buf,
//                            int n,
